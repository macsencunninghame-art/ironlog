...
import { CATEGORY_NAMES, categoriesIn, categoryOf, dayIds, type Category } from '@/lib/routine'
...
export function HistoryPage() {
  ...
  const categories = categoriesIn(routine)

  // The two filters stack: the day picks the sessions, the category picks what is
  // shown inside them - and drops any session with nothing of that category in it.
  const visible = workouts
    .filter((w) => filter === 'all' || w.dayId === filter)
    .filter(
      (w) =>
        category === 'all' ||
        w.exercises.some(
          (ex) =>
            (ex.sets.some((s) => s.done) || ex.warmupsDone.some(Boolean)) &&
            categoryOf(routine, w.dayId, ex.exerciseId) === category,
        ),
    )
      ...
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="num mt-0.5 text-sm text-chalk-muted">
            {workouts.length} session{workouts.length === 1 ? '' : 's'} · {fmtVolume(totalVolume)}{' '}
            kg lifted
          </p>
        </div>
      </div>
        ...
        <>
          ...
          {visible.length ? (
            <div className="space-y-3">
              {visible.map((workout) => (
                <SessionCard key={workout.id} workout={workout} category={category} />
              ))}
            </div>
          ) : (
              ...
              <p className="max-w-[260px] text-xs font-medium text-chalk-faint">
                {!workouts.length
                  ...
                  : category === 'all'
                    ...
                    : `No ${CATEGORY_NAMES[category]} work logged for this day yet.`}
              </p>
            </Card>
          )}
          ...
          <Card className="p-5">
            ...
            <p className="mt-1 text-[11px] leading-relaxed text-chalk-muted">
              Everything lives in this browser only. Clearing your browsing data wipes it. Export a
              copy now and again, and keep it somewhere safe. Nothing can be deleted from inside the
              app — importing is the one exception, and it replaces this person&apos;s whole log.
            </p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[11px] leading-relaxed text-amber-700/80">
                      Importing <span className="font-semibold">{pendingFile.name}</span> overwrites
                      all {workouts.length} logged session{workouts.length === 1 ? '' : 's'} and
                      every 1RM entry. This cannot be undone.
                    </p>
